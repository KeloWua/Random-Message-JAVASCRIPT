...
const frases = unorderedSongs.map(group => group[Math.floor(Math.random() * group.length)]).join('<br><br>');
//We print them.
      const box = document.getElementById('box');
      box.innerHTML = '';

      const nuevoParrafo = document.createElement('p');
      nuevoParrafo.innerHTML = frases;
      box.appendChild(nuevoParrafo);
console.log('Button clicked, message added.');
    }
